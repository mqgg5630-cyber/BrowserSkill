...
while IFS= read -r -d '' f; do
    ...
    -not -path './install.ps1' -not -path './scripts/*' -print0)
# note: ./install.ps1 and ./scripts/*.ps1 belong to BrowserSkill itself (they
# predate the git-sync skill and legitimately contain non-ASCII text); the
# git-sync copy lives at ./git-sync-install.ps1.
if [ "$fail" -eq 0 ]; then
...
drift=0
# BrowserSkill already ships its own install.ps1, so the skill's copy is kept
# at git-sync-install.ps1 instead of overwriting it.
for f in sync push upload download pack doctor bootstrap pr hardware watch auth git-sync-install; do
    src="$f"
    [ "$f" = "git-sync-install" ] && src="install"
    if [ -f "skills/git-sync/scripts/$src.ps1" ]; then
        if [ ! -f "$f.ps1" ]; then
            echo "[FAIL] $f.ps1 is missing at the repo root (the skill ships it)"
            drift=1
        elif ! cmp -s "$f.ps1" "skills/git-sync/scripts/$src.ps1"; then
            echo "[FAIL] $f.ps1 differs from skills/git-sync/scripts/$src.ps1 - copy it over"
            drift=1
        fi
    fi
done
