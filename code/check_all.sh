...
# ---------------------------------------------------------------- 1. ps1
# Repo-owned exception (BrowserSkill): scripts/install-windows.test.ps1 is a
# Unicode-handling TEST FIXTURE. It carries a UTF-8 BOM and CJK path literals
# on purpose, to prove the repo's own install.ps1 does not lose them. Making it
# ASCII would delete exactly what it tests, so it is skipped here.
# install.ps1 is BrowserSkill's own bsk-CLI installer (a UTF-8 em dash in its
# comment header) - also repo-owned, also not a skill file, so also skipped.
# Both are listed in section 3's ROOT_SKIP; every .ps1 the git-sync skill ships
# is still checked, with no exceptions.
ASCII_ALLOWLIST=(
...
ascii_allowed() {
    local p="$1" a
    for a in ${ASCII_ALLOWLIST[@]+"${ASCII_ALLOWLIST[@]}"}; do
        [ "$p" = "$a" ] && return 0
    done
    return 1
}
while IFS= read -r -d '' f; do
    if LC_ALL=C grep -qn '[^[:print:][:space:]]' "$f"; then
        if ascii_allowed "$f"; then
            echo "NOTE: non-ASCII allowed in $f (repo-owned file - see the ASCII_ALLOWLIST comment above)"
        else
...
# Repo-owned exception (BrowserSkill): the repo root ALREADY ships its own
# install.ps1 (the bsk CLI installer, `irm .../install.ps1 | iex`). The git-sync
# copy of install.ps1 therefore lives only at skills/git-sync/scripts/install.ps1
# and is run by full path:
#     .\skills\git-sync\scripts\install.ps1 -Target <dir> -Branch <branch>
# agent-install.sh copies it over the root file again on every (re-)install, so
# after any upgrade run:  git checkout -- install.ps1
ROOT_SKIP="install"
drift=0
for f in sync push upload download pack doctor bootstrap pr hardware watch auth install; do
    skip=0
    for s in $ROOT_SKIP; do
        [ "$s" = "$f" ] && skip=1
    done
    if [ "$skip" = "1" ]; then
        # Not compared - but the opposite drift IS an error: agent-install.sh
        # copies the skill's install.ps1 over this repo's own file on every
        # (re-)install, which would silently replace the bsk CLI installer.
        if [ -f "$f.ps1" ] && [ -f "skills/git-sync/scripts/$f.ps1" ] \
           && cmp -s "$f.ps1" "skills/git-sync/scripts/$f.ps1"; then
            echo "[FAIL] root $f.ps1 was overwritten by the skill copy - it belongs to THIS repo."
            echo "       restore it:  git checkout -- $f.ps1"
            drift=1
        else
            echo "NOTE: root $f.ps1 belongs to THIS repo - the skill copy stays at skills/git-sync/scripts/$f.ps1 (run it by full path)"
        fi
        continue
    fi
